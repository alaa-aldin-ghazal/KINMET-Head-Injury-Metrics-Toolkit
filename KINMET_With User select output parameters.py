#!/usr/bin/env python3
"""
head_injury_report_gui_final.py

Integrated, publish-ready GUI tool to compute head injury indicators from 6-DOF kinematics.
Features:
 - Column mapping + units dialog with live preview (raw vs filtered overlay)
 - Filter selection (CFC / Butterworth / Savitzky-Golay / Moving Average) inside mapping dialog
 - Preview shows estimated sampling frequency and recommended CFC
 - GUI warnings when filter cutoff invalid (Nyquist) and disables irrelevant parameter entries
 - Compute HIC15, HIC36, Gadd SI, BrIC, peak accel/omega, neck loads & Nij (estimated)
 - Generate multi-page PDF report (tables + plots + methods)
 - Save metrics JSON with processing metadata
License: MIT
"""
...
# ---------- Report section selection dialog ----------
REPORT_SECTIONS = {
    "summary_table": "Summary table (metrics, risk, AIS)",
    "linear_accel_plot": "Linear acceleration time series",
    "angular_vel_plot": "Angular velocity time series",
    "angular_accel_plot": "Angular acceleration time series",
    "hic_windows_plot": "HIC15/HIC36 windows",
    "bric_curve_plot": "BrIC over time",
    "wstc_curve_plot": "WSTC probability curve",
    "neck_forces_plot": "Neck forces (Fx, Fy, Fz)",
    "neck_moments_plot": "Neck moments (Mx, My, Mz)",
    "nij_plot": "Nij time history",
    "ais_equations": "AIS probability equations & bar chart"
}

def ask_report_sections(parent=None) -> list:
    """
    Show a checklist dialog for selecting which sections to include in the PDF report.
    Returns a list of selected section keys (e.g., ['summary_table', 'linear_accel_plot']).
    If user cancels, returns all sections (default).
    """
    result = {"sections": list(REPORT_SECTIONS.keys())}  # default all
    win = tk.Toplevel(parent) if parent else tk.Toplevel()
    win.title("Select Report Sections")
    win.geometry("500x450")
    win.attributes("-topmost", True)
    win.update()
    win.attributes("-topmost", False)

    tk.Label(win, text="Choose which components to include in the PDF report:", font=("Helvetica", 10, "bold")).pack(pady=8)
    ...
    check_vars = {}
    for key, desc in REPORT_SECTIONS.items():
        var = tk.BooleanVar(value=True)  # default selected
        check_vars[key] = var
        chk = tk.Checkbutton(scrollable_frame, text=desc, variable=var, anchor="w")
    ...
    def on_confirm():
        result["sections"] = [k for k, v in check_vars.items() if v.get()]
        win.destroy()
    ...
    tk.Button(btn_frame, text="Select All", command=on_select_all).pack(side="left", padx=5)
    tk.Button(btn_frame, text="Deselect All", command=on_deselect_all).pack(side="left", padx=5)
    tk.Button(btn_frame, text="Confirm", command=on_confirm).pack(side="left", padx=5)
    tk.Button(btn_frame, text="Cancel (use all)", command=win.destroy).pack(side="left", padx=5)
...
def generate_report(time: np.ndarray, ax_g: np.ndarray, ay_g: np.ndarray, az_g: np.ndarray,
                    ...
                    out_pdf: str, in_file_name: str = "", processing_metadata: dict = None,
                    sections: list = None) -> Dict[str, Any]:
    """
    ...
    # Filter out any invalid keys, but keep empty list if user deselected all
...
#    pdf.savefig(fig); plt.close(fig)
    ...
    if "linear_accel_plot" in sections:
    ...
    if "angular_vel_plot" in sections:
        fig, ax = plt.subplots(figsize=(11,4))
    ...
    if "angular_accel_plot" in sections:
        fig, ax = plt.subplots(figsize=(11,4))
    ...
    if "hic_windows_plot" in sections:
        fig, ax = plt.subplots(figsize=(11,4))
    ...
    if "bric_curve_plot" in sections:
        fig, ax = plt.subplots(figsize=(11,4))
    ...
    if "wstc_curve_plot" in sections:
        fig, ax = plt.subplots(figsize=(11,4))
    ...
    if "neck_forces_plot" in sections:
    ...
    if "neck_moments_plot" in sections:
        fig, axp = plt.subplots(figsize=(11,6))
...
def main():
    ...
    try:
        ...
        if not out_pdf:
            messagebox.showerror("No output", "No output file selected. Exiting.")
            return

        # Ask which report sections to include
        selected_sections = ask_report_sections(root)
